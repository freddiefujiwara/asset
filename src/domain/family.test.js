...
describe("family domain", () => {
  ...
  it("summarizes holdings by owner", () => {
    const groups = summarizeFamilyAssets({
      cashLike: [{ 種類・名称: "普通預金@chipop", 残高: "100" }],
      stocks: [{ 銘柄名: "ETF@aojiru.pudding", 評価額: "200" }],
      funds: [{ 銘柄名: "All Country", 評価額: "300" }],
      pensions: [],
      points: [],
    });

    expect(groups.find((g) => g.ownerLabel === "妻")?.totalYen).toBe(100);
    expect(groups.find((g) => g.ownerLabel === "娘")?.totalYen).toBe(200);
    expect(groups.find((g) => g.ownerLabel === "私")?.totalYen).toBe(300);
  });
});
